...
- **特点**: 教师数据通过 JavaScript 数组 `teachers_students` 动态渲染，博士生通过独立表格页面展示

### 人员入口

- **主页面**: `CH.People.ashx`（锚点导航: `#director` 负责人 / `#faculty` 教师 / `#visitprof` 访问教授）
- **博士生**: `CH.PhD_student.ashx`
- **硕士生**: `CH.MSc_students.ashx`
- **博士后**: `CH.postdoctoral_fellow.ashx`（通常为空）
- **校友**: `CH.previous_people_alumni.ashx`
- **教师 JS 数据**: 页面内 `let teachers_students = [...]` 包含教师及学生关系

### 采集策略

#### 教师提取（JS 数组）
教师/学生数据不渲染在 DOM 中，需从 HTML 源码提取 JS 数组并用 Python 解析：
```python
# 1. 正则提取 JS 数组字符串
pattern = r'let teachers_students\s*=\s*(\[[\s\S]*?\])\s*;'
match = re.search(pattern, html)

# 2. 解析 JS 对象（key 无引号）用自定义解析器
# 详见 sessions 中的 parse_teachers.py 实现
```

#### 博士生提取（表格）
使用 `browser_console` 执行 JS 一次性提取所有 94 人，包含姓名/URL/入学年份：
```javascript
// 过滤出 lamda.nju 个人页面链接，排除导航和 [English] 链接
const allLinks = document.querySelectorAll('td a');
allLinks.forEach(a => {
  // 过滤条件: lamda域内、非CH/Edit/MainPage路径、文字<20字符
  const text = a.textContent.trim();
  const href = a.href || '';
  if (href.includes('lamda.nju.edu.cn') && text && text.length < 20
      && !href.includes('CH.') && !href.includes('Edit.') && !href.includes('MainPage')) {
    const year = tdText.match(/(\d{4})\s*-/)?.[1];
    data.push({name: text, url: href, cohort_year: parseInt(year)});
  }
});
```

#### Bio 并发 HTTP 提取
个人主页为**静态 HTML**，用 urllib + ThreadPoolExecutor 并发爬取（94 人约 30 秒）：
```python
...
**关键提取模式：**
- **role_raw**: 正则 `(Ph\.D\.\s+(?:student|Student|candidate|Candidate))` 或 `(Professor|Associate Professor|Postdoc)`
- **email（直接）**: `Email:\s*([\w.%+-]+@[\w.-]+\.[A-Za-z]{2,})`
- **email（混淆）**: `([\w.-]+)\(at\)([\w.-]+)\(dot\)([\w.-]+)` → 拼接
- **cohort_year**: Education 段或 `admitted/enrolled/started/joined in YYYY`
- **research_areas**: `About Me` 中 `include|focus on|interested in` 后逗号分隔内容
- **advisor**: `Supervisor:\s*(?:Prof\.?\s*)?([^<\n]+?)(?:Co-supervisor|Email|$)`

#### 个人主页 URL 模式
- **教师**: `http://www.lamda.nju.edu.cn/<pinyin>`（无尾部斜杠）或 `cs.nju.edu.cn/xxx`、`ai.nju.edu.cn/xxx`
- **博士生**: `http://www.lamda.nju.edu.cn/<pinyin-slug>/`（有尾斜杠，如 `liuyr/`）
- URL 用**拼音缩写**，不能按中文名猜，必须从页面提取完整链接列表

### 注意事项
1. **JS 数组解析**: `key: "value"` 语法非标准 JSON，需写自定义解析器（不能用 `json.loads`）
2. **外部域名教师**: cs.nju.edu.cn / ai.nju.edu.cn 页面 HTML 结构不同
3. **空页面**: 博士后/访问教授页面可能空，快速跳过
4. **LAMDA-RL 小组**: 博士生已在主列表中，避免重复
5. **教师 URL 重定向**: `gaow` → `gaow/`，两种形式都试
...
- **导航**: 图片式表格导航，通过 `onclick="window.location='CH.People.ashx'"` 跳转
- **Robots.txt**: 无 robots.txt 限制，可直接爬取
- **特点**: 中国高校实验室典型结构，教师数据通过 JavaScript 数组动态渲染
...
[
  {
    ...
    students: {
      doctors: [{name: "钱梦璋", link: "...", year: "2022"}, ...],
